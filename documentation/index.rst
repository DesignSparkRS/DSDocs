...
build pipeline and the resulting HTML is hosted via their servers. However, before
this is done the site should be generated locally, so as to verify formatting
and avoid continually pushing, having to wait for the site to build and risking
errors going live. Only when the local build looks good do we commit and push.
...
Obviously projects such as this one that are purely documentation, will have only
the Sphinx config and content in their repo.
